...
ha decidido remplazar la implementación actual del sistema de gestión en COBOL por una más moderna en Go.
...
Garantías (pre-condiciones):
  ...
  - los años de inscripción son números enteros positivos en base 10, y
    ningun paciente se inscribió luego del presente año, ni antes del año 0
...
Para ejecutar las pruebas correr (similar al TP1):
...
El sistema debe implementar los tres comandos que se detallan a continuación.
...
paciente actual (que puede ser 0). Esto incluye urgentes como regulares. 
...
especificados, se proporciona un archivo `mensajes.go` que contiene las
...
La manera de usarlos, entonces, es:
        ...
        import "fmt"
        import "mensajes"
        fmt.Printf(mensajes.PACIENTE_ENCOLADO, "Ingeniero Barrios");

Si hay alguna discrepancia entre esta consigna y el archivo `mensajes.go`, este
último es la versión canónica.

#### Otras condiciones de error


El sistema aborta la ejecución ante cualquiera de los siguientes
...
Se proporciona el archivo `mensajes.go` para la estandarización de los mensajes
de salida. 
...
Es necesario emplear la estructura de datos más apropiada para cada funcionalidad del
...
posible y correctamente documentadas. Deben poder emplear sin modificar
...
- El código de los TDAs utilizados, que deben estar en el módulo `tdas` tal como venimos implementando.
- El código del TP en un directorio llamado `tp2` (el módulo puede llamarse como deseén, pero el directorio debe llamarse `tp2`). Recordar revisar [el video provisto para la organización de módulos para la utilización de los TDAs](https://youtu.be/IZrQt-vR3E4?t=585) en el trabajo práctico.
