...
1.  (★★★) A lo largo de su trayectoria, la empresa ha tenido varias rotaciones de
    equipos. Próximamente habrá una nueva, y se busca que los nuevos equipos
    incluyan personas que hayan compartido equipos antes, para facilitar la
    transición.

    Se dispone de una base de datos que registra, para `N` personas, en qué `M` equipos ha
    participado cada una. La base usa un hash donde la persona es la clave y el valor,
    ...
    Realizar una función `personasEnComun` que reciba el hash, y el nombre 
    de una persona , y devuelva una lista con todas las personas que hayan 
